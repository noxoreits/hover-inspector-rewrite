...
namespace Mod
{
    ...
    {
        // Permeance Toggle
        public static KeyCode togglePermeance = KeyCode.LeftAlt; // LeftAlt default if config is not found
        ...
        {   
            ...
            if (Input.GetKeyDown(togglePermeance))
            {
                keepUIOpen = !keepUIOpen;
                ModAPI.Notify(("Inspect on Hover Permeance: "+keepUIOpen).ToString());
            }
            ...
            // Functionality for the permeance keybind incorporated into the null check
            if (target == null) {
                if (keepUIOpen){
                    return;
                }
        ...
        private void LoadConfig() {
            ...
            try{
                if (File.Exists(configLocation)){
                    ...
                    if (config == null || string.IsNullOrEmpty(config.TogglePermeanceKey))
                    {
                        togglePermeance = KeyCode.LeftAlt; // Fallback for if the json is wrong
                        ModAPI.Notify("Hover Inspector - Config invalid, falling back to LeftAlt");
                    }
                    else
                    {
                        togglePermeance = (KeyCode)Enum.Parse(typeof(KeyCode), config.TogglePermeanceKey, true);
                        ModAPI.Notify("Hover Inspector key: " + togglePermeance);
                        keepUIOpen = config.IsPermeanceEnabled;
                    }
                }
                else{ // In case config is deleted
                    ConfigData defaultConfig = new ConfigData { TogglePermeanceKey = "LeftAlt", IsPermeanceEnabled = true }; // Set key to LeftAlt automatically as default and the permeance default too
                ...
                togglePermeance = KeyCode.LeftAlt;
            }
        }
        ...
        private void SaveConfig(){
            ...
            ConfigData defaultConfig = new ConfigData { TogglePermeanceKey = togglePermeance.ToString(), IsPermeanceEnabled = keepUIOpen }; // Keybind stays the same, save state of keepUIOpen for future
            string json = JsonConvert.SerializeObject(defaultConfig, Formatting.Indented); // Reserialise cause unity shit i guess im bad at json
            File.WriteAllText(configLocation, json); // Save new config.json yaaaaaaayyyyyyyy
        }
    ...
    {
        public string TogglePermeanceKey;
        public bool IsPermeanceEnabled;
    }
}